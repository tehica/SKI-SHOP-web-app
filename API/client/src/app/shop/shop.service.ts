...
export class ShopService {
  ...
  getProduct(useCache: boolean) {
    ...
    if (this.products.length > 0 && useCache === true) {
      ...
      //check what page numbet is being requested
    ...
    // pipe allows us to chain multiple js operators to manipulate or do
    // something with the observable as it comes back in.
    return this.http.get<IPagination>(this.baseUrl + 'products', { observe: 'response', params })
      .pipe(
        map(response => {
          // this is going to append new set of results from an API along with the existing set of results
          // from the API
  ...
  getProductDetails(id: number) {
    ...
    // of means we returning an observable of something
    if (product) {
      return of(product);
    }
